...
3. **Rules compliance** — the project's rule files (`CLAUDE.md`, `.claude/rules/`) are followed.
...
3. **Rules compliance** — audit against the discovered rule files. A `CLAUDE.md` applies only to files it shares a path with (the file or its parents). Files under `.claude/rules/` apply repo-wide unless the rule itself scopes them. Flag only clear, unambiguous violations where the exact rule and its source file path can be quoted.
